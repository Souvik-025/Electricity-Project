...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
	...
	List<Map<String, Object>> findSelectiveCustomerDetailsAsMap(@Param("adminId") Integer adminId);
	...
	Page<Customer> searchByAdminAndTerm(@Param("adminId") Integer adminId, @Param("search") String search,
			Pageable pageable);
	...
	@Query("SELECT c FROM Customer c WHERE c.admin.adminId = :adminId AND c.isVerified = :isVerified AND ("
			...
			+ "LOWER(c.mobileNumber) LIKE LOWER(CONCAT('%', :search, '%')) OR "
			+ "LOWER(c.companyName) LIKE LOWER(CONCAT('%', :search, '%'))" + ")")
	Page<Customer> searchVerifiedCustomers(@Param("adminId") Integer adminId, @Param("search") String search,
	...
	@Query("SELECT c FROM Customer c WHERE c.admin.adminId = :adminId AND c.userType = :userType AND ("
			...
			+ "LOWER(c.mobileNumber) LIKE LOWER(CONCAT('%', :search, '%')) OR "
			+ "LOWER(c.companyName) LIKE LOWER(CONCAT('%', :search, '%'))" + ")")
	Page<Customer> searchByUserTypeAndTerms(@Param("adminId") Integer adminId, @Param("search") String search,
	...
	@Query("SELECT c FROM Customer c WHERE c.admin.adminId = :adminId AND c.userType = :userType AND c.isVerified = :isVerified AND ("
			...
			+ "LOWER(c.mobileNumber) LIKE LOWER(CONCAT('%', :search, '%')) OR "
			+ "LOWER(c.companyName) LIKE LOWER(CONCAT('%', :search, '%'))" + ")")
	Page<Customer> searchByUserTypeAndVerifiedAndTerms(@Param("adminId") Integer adminId,
